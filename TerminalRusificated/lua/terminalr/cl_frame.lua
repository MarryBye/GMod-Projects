...
function TERMINAL:PaintOver()
	if ( TerminalR.Programs[self.Key].PaintOver ) then
...
function TERMINAL:CreateMenuButton( index, text )
	...
	DButton.Paint = function( selfB, w, h )
		...
		local cclr = Color(math.random(cr - 35, cr), math.random(cg - 35, cg), math.random(cb - 35, cb), selfB.Alpha * 2)
		draw.SimpleText( text, "TrmR_Medium", 20, 5 + math.random( -H*0.005, H*0.005 ), cclr, 0, 0 )
		draw.SimpleText( text, "TrmR_Medium", 20, 5, Color(cclr.r, cclr.g, cclr.b, math.random(230, 255)), 0, 0 )
		draw.RoundedBox( 0, 0, 0, w, h, Color(math.random(cr - 35, cr), math.random(cg - 35, cg), math.random(cb - 35, cb), selfB.Alpha) )
	end
