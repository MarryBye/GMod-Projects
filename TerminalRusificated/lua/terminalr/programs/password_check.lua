...
TerminalR.Programs["password_check"].Paint = function( self, w, h )
	...
	draw.SimpleText( "Введите пароль", "TrmR_Large", w*0.5, h*0.35, cclr, 1, 0 )
	local RNum = math.random(210,255)
	draw.SimpleText( "> " .. self:GetMemory("Password") .. " <", "TrmR_Large", w*0.5, h*0.45, cclr, 1, 0 )
	draw.SimpleText( "НЕВЕРНЫЙ ПАРОЛЬ", "TrmR_Medium", w*0.5, h*0.3, Color( RNum, 0, 0, self:GetMemory("WrongAlpha") ), 1, 0 )
end
