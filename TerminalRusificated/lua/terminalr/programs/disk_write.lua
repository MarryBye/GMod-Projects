...
TerminalR.Programs["disk_write"].Init = function( self )
	...
	ParentTable["TE"]:SetUpdateOnType(true)
	...
	ParentTable["TE"].OnValueChange = function( selfD, stringValue )

		if input.IsKeyDown( 73 ) then return end
		if input.IsKeyDown( 64 ) then return end
		if input.IsKeyDown( 66 ) then return end

		local Text = self:GetMemory("DiskText")[self:GetMemory("CharLine")]
		local CharX = self:GetMemory( "CharPosX" )

		local NewText = utf8.sub( Text, 0, CharX ) .. utf8.sub(stringValue, utf8.len(stringValue)) .. utf8.sub( Text, CharX + 1, 9000 )
	...
	ParentTable["TE"].Think = function( selfD )
		if ( CurTime() > selfD.NextTyping ) then
			...
			if ( input.IsKeyDown( 73 ) ) then -- Suppr
				...
				local CharX = self:GetMemory( "CharPosX" )
				if ( utf8.sub( Text, CharX + 1, CharX + 1 ) == "" and CharX == 0 ) then
				...
				local NewText = utf8.sub( Text, 0, CharX ) .. utf8.sub( Text, CharX + 2, 9000  )
				self:GetMemory("DiskText")[self:GetMemory("CharLine")] = NewText
				LocalPlayer():EmitSound("terminalr/char/" .. math.random( 1, 5 ) .. ".wav" )
				selfD.NextTyping = CurTime() + 0.1
			...
			elseif ( input.IsKeyDown( 66 ) ) then -- Delete
				...
				local NewText = utf8.sub( Text, 0, CharX -1 ) .. utf8.sub( Text, CharX+1, 9000  )
...
TerminalR.Programs["disk_write"].Paint = function( self, w, h )
	...
	local PDD = utf8.sub( Text, 0, CharX  )
	local PDDT = surface.GetTextSize( PDD )
	local CS = utf8.sub( Text, CharX + 1, CharX + 1 )
	local TX, TY = surface.GetTextSize( CS )
	-- local AVS = utf8.sub( Text, 0, CharX )
	-- local APS = utf8	.sub( Text, CharX + 2, 9000  )
	...
	draw.RoundedBox( 0, w*0.05 + PDDT, (h*0.05 + ( self:GetMemory("CharLine") - 1 ) * h*0.05) - self:GetMemory("CamY") , TX, TY, cclr )
